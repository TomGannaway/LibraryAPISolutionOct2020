...
namespace LibraryAPI.Controllers
{
    public class BooksController : ControllerBase
    {
        ...
        {
            var savedBook = await _context.Books.SingleOrDefaultAsync(b=>b.Id == id);
            if (savedBook != null)
            {
                savedBook.IsInInventory = false;
                await _context.SaveChangesAsync();
            }
            return NoContent(); // 204 - means 'okay', but don't look for any data. i get nothing.
        }

        [HttpPost("books")]
        public async Task<ActionResult<GetBookDetailsResponse>> AddBook([FromBody] PostBooksRequest bookToAdd)
        {
